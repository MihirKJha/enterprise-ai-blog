...
### Diagram 2.1 — Enterprise AI Failure and Recovery

```mermaid
graph TD
    R[AI Request] --> A[Authorization]
    A -->|Allowed| P[Primary Execution]
    A -->|Denied| X[Controlled Rejection]
    P --> V[Validation]
    V --> S{Success?}
    S -->|Yes| RESP[Response]
    S -->|No| F[Failure Classification]
    F --> T{Failure Type}
    T -->|Transient| RT[Retry]
    T -->|Capability Failure| FB[Approved Fallback]
    T -->|Quality Failure| REC[Recovery Path]
    T -->|Unsafe| STOP[Controlled Stop]
    T -->|Automation Boundary| HE[Human Escalation]
    RT --> P
    FB --> A2[Re-Authorization]
    A2 --> V
    REC --> V
    HE --> H[Human Decision]
```
...
```mermaid
graph TD
    R[AI Request] --> P[Primary AI Path]
    P --> M[Primary Model]
    P --> K[Primary Knowledge]
    P --> T[Primary Tools]
    M --> F{Failure}
    K --> F
    T --> F
    F -->|Healthy| V[Validation]
    F -->|Failure| D[Recovery Policy]
    D --> M2[Alternative Model]
    D --> K2[Alternate Knowledge]
    D --> T2[Alternate Tool]
    D --> H[Human Escalation]
    M2 --> C[Control Boundary]
    K2 --> C
    T2 --> C
    H --> C
    C --> V
    V --> G[Guardrails + Output Validation]
    G --> RESP[Controlled Response]
```
...
### Diagram 2.3 — Retrieval Failure and Evidence Control

```mermaid
graph TD
    Q[User Query] --> R[Retrieval]
    R --> E{Evidence Quality}
    E -->|Strong| C[Context Assembly]
    E -->|Weak| RR[Recovery Retrieval]
    E -->|None| D[Controlled Degradation]
    E -->|Source Failed| AS[Alternate Source]
    E -->|Unauthorized| STOP[Controlled Stop]
    RR --> E
    AS --> E
    C --> V[Evidence Validation]
    V --> G[Generation]
    G --> O[Output Validation]
    O --> RESP[Response]
```
...
### Diagram 2.4 — Recovery Decision Policy
 ...
 ```mermaid
 graph TD
     F[Failure] --> C[Classify Failure]
     C --> T{Transient?}

     T -->|Yes| R[Retry with Limits]
     T -->|No| A{Approved Alternative?}

     A -->|Yes| FB[Fallback]
     A -->|No| D{Safe to Degrade?}

     D -->|Yes| GD[Graceful Degradation]
     D -->|No| H{Safe to Continue?}

     H -->|Yes| V[Validate]
     H -->|No| HE[Human Escalation]

     R --> V
     FB --> V
     GD --> V
     HE --> HD[Human Decision]
 ```
