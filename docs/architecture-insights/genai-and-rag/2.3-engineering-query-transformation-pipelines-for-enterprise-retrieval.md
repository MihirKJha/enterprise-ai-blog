...
# 2.3 — Engineering Query Transformation Pipelines for Enterprise Retrieval
...
graph TD
    ...
    B[Query<br/>Analysis]
    C[Transformati<br/>on<br/>Strategy]
...
graph TD
    A[User<br/>Query]
    B[Transformati<br/>on]
    C[Retrieval<br/>Query A]
    D[Retrieval<br/>Query B]
    E[Retrieval<br/>Query C]
    F[Retrieval<br/>Query D]
    G[Candidate<br/>Fusion]
    H[Final Candidate<br/>Set]
...
## 13. Implementing Query Transformation & Retrieval Orchestration

The transformation layer can dynamically select one or more query transformation strategies. Each strategy implements a common interface, allowing the architecture to add, replace, or combine strategies without changing the application layer.

### Query Transformation Interface

```java
interface QueryTransformer {

    List<RetrievalQuery> transform(
        UserQuery query,
...
Different transformation strategies implement the same capability:
...
```java
class RewriteTransformer implements QueryTransformer {

    @Override
    public List<RetrievalQuery> transform(
        UserQuery query,
        RetrievalContext context
    ) {
...
```python
def select_strategies(query):
    strategies = []

    if query.is_complex:
        strategies.append(decomposition)

    if query.has_ambiguous_terminology:
        strategies.append(rewriting)

    if query.requires_broad_coverage:
        strategies.append(expansion)

    if query.requires_multiple_perspectives:
        strategies.append(multi_query)

    if not strategies:
        strategies.append(semantic_retrieval)
...
```python
strategies = select_strategies(query)

retrieval_queries = []

for strategy in strategies:
    retrieval_queries.extend(
        strategy.transform(query)
    )
```
...
Multiple strategies may return the same evidence. Candidates therefore need to be deduplicated and fused into a common ranked set.
...
fused_candidates = fuse_candidates(candidates)

context = fused_candidates[:10]
...
graph TD
    ...
    B --> C[Dynamic Strategy Selection]
...
This keeps **transformation strategy, retrieval execution, candidate fusion, and generation** as separate capabilities. New transformation strategies can be introduced without changing the application contract.
