...
# Architecting Retriever Selection: An Enterprise Decision Framework
...
This chapter builds on the retrieval capabilities covered earlier in the Enterprise AI Systems Architecture Stack:
...
graph TD
    ...
    B --> C[Corpus Characteristics]
    C --> D[Metadata + Document Hierarchy]
    D --> E[Precision / Recall]
    E --> F[Latency + Cost]
    F --> G[Retriever Strategy]
```
...
```mermaid
graph TD
    Q[Enterprise Query] --> S[Semantic Retrieval]
    Q --> M[Metadata-Aware Retrieval]
    Q --> MQ[Multi-Query Retrieval]
    Q --> P[Parent-Document Retrieval]

    S --> C[Candidate Evidence]
    M --> C
    MQ --> C
    P --> C

    C --> V[Retrieval Validation / Ranking]
```
...
graph TD
    ...
    Q --> B[Structured Constraints]
    Q --> C[Multiple Interpretations]
    Q --> D[Complex Multi-Part Question]
    Q --> E[Document Context Required]

    A --> R1[Semantic Retrieval]
    B --> R2[Metadata / Self-Query]
    C --> R3[Multi-Query Retrieval]
    D --> R4[Query Transformation / Decomposition]
    E --> R5[Parent-Document Retrieval]
```
